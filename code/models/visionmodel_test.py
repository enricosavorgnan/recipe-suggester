...
# Internal imports
from runmodel import FridgeDetector 
...
class TestVisionPipeline(unittest.TestCase):
    ...
    def test_black_image(self):
        ...
        model = YOLO("runs/detect/yolo_frigo_v14/weights/best.onnx")
    ...
    def test_detection_accuracy_items(self):
        ...
        img = cv2.imread(img_path)
        expected_labels = sorted(['Tomato', 'Bell pepper', 'Beetroot', 'Lettuce'])
        print('cias')
        # Call the detection model prediction procedure
        model = YOLO("runs/detect/yolo_frigo_v14/weights/best.onnx")
        result = model.predict(
        ...
        self.assertEqual(len(result[0].boxes), 4, "Wrong number of items detected ")

        # Check if the detected items were labeled correctly 
        #detected_labels = sorted([item['label'] for item in result[0].boxes])
        self.assertListEqual(detected_labels, expected_labels, "Le classi rilevate non sono corrette")
    ...
    def test_bounding_box_area(self):
        ...
        for i, (x1, y1, x2, y2) in enumerate(boxes):
            ...
            self.assertGreater(coverage_perc, 0.5, f"Too small item: {label} ({coverage_perc:.2f}%)" )
